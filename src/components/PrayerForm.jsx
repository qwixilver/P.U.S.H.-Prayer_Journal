...
// - Archived requestors are excluded from prayer assignment.
...
export default function PrayerForm({ initialPrayer, onSuccess, onCancel }) {
  ...
  const [name, setName] = useState(
    isEdit ? initialPrayer.name || '' : ''
  );
  const [description, setDescription] = useState(
    isEdit ? initialPrayer.description || '' : ''
  );
  const [requestedAt, setRequestedAt] = useState(
    isEdit
      ? initialPrayer.requestedAt
        ? initialPrayer.requestedAt.slice(0, 10)
        : new Date().toISOString().slice(0, 10)
      : new Date().toISOString().slice(0, 10)
  );
  const [status, setStatus] = useState(
    isEdit ? initialPrayer.status || 'requested' : 'requested'
  );
  const [answeredAt, setAnsweredAt] = useState(
    isEdit && initialPrayer.answeredAt
      ? initialPrayer.answeredAt.slice(0, 10)
      : ''
  );
  const [security, setSecurity] = useState(
    isEdit ? Boolean(initialPrayer.security) : false
  );
  const [busy, setBusy] = useState(false);
  const [err, setErr] = useState('');

  // Load categories + active requestors once.
  useEffect(() => {
    (async () => {
      ...
      const activeRequestors = reqs.filter(
        (requestor) => !Boolean(requestor.archived)
      );

      setCategories(cats);
      setAllRequestors(activeRequestors);

      // If editing, derive initial category from the prayer's active requestor.
      if (isEdit && initialPrayer.requestorId) {
        const req = activeRequestors.find(
          (requestor) => requestor.id === initialPrayer.requestorId
        );
  ...
  const requestors = useMemo(() => {
    ...
    return allRequestors.filter(
      (requestor) => requestor.categoryId === Number(categoryId)
    );
  ...
  useEffect(() => {
    ...
    const ok = requestors.some(
      (requestor) => requestor.id === Number(requestorId)
    );
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      console.error('Prayer save failed', saveError);
    ...
    <form onSubmit={handleSubmit} className="p-4 bg-gray-700 rounded-lg">
      ...
      <div className="flex items-center mb-2">
        <input
          ...
          className="mr-2"
        />
        <label htmlFor="pf-security" className="text-gray-200 text-sm">
          Security Only
        </label>
      </div>
